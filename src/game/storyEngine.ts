...
export function careerStory(career: Career): string[] {
  ...
  if (r.academyGraduate && r.appearances >= 10) {
    ...
    lines.push(
      'עשית את כל הדרך במחלקת הילדים והנוער של מכבי חיפה, ובגיל 18 אמרו לך שזה לא יקרה כאן.',
    );
  } else {
    lines.push('גדלת במחלקת הילדים והנוער של מכבי חיפה, ומשם הקריירה לקחה אותך למקום אחר.');
  }
